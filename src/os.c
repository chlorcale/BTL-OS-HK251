...
static void read_config(const char * path) {
    ...
    if (fscanf(file, "%d %d %d", &time_slot, &num_cpus, &num_processes) != 3) {
        printf("ERROR: Invalid config format in header line!\n");
        exit(1);
    }
    ...
    for (int i = 0; i < num_processes; i++) {
        ld_processes.path[i] = malloc(256);
        ld_processes.path[i][0] = '\0';

#ifdef MLQ_SCHED
        rv = fscanf(file, "%lu %s %lu",
                    &ld_processes.start_time[i],
                    proc_name,
                    &ld_processes.prio[i]);
        if (rv != 3) {
            printf("ERROR: Invalid process entry at line %d\n", i+2);
            exit(1);
        }
#else
        rv = fscanf(file, "%lu %s",
        ...
        /* Build path: input/proc/<name> */
        strcat(ld_processes.path[i], "input/proc/");
        strcat(ld_processes.path[i], proc_name);
    }
